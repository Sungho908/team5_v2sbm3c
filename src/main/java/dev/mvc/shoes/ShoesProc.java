...
public class ShoesProc implements ShoesProcInter {
  ...
  public int update_seqno_forward(int shoesno) {
    int cnt = this.shoesDAO.update_seqno_forward(shoesno);
    return cnt;
  }

  @Override
  public int update_seqno_backward(int shoesno) {
    int cnt = this.shoesDAO.update_seqno_backward(shoesno);
    return cnt;
  ...
  public int update_visible_y(int shoesno) {
    int cnt = this.shoesDAO.update_visible_y(shoesno);
    return cnt;
  }

  @Override
  public int update_visible_n(int shoesno) {
    int cnt = this.shoesDAO.update_visible_n(shoesno);
    return cnt;
  }

  @Override
  public ArrayList<ShoesVO> list_all_name_y() {
    ArrayList<ShoesVO> list = this.shoesDAO.list_all_name_y();
    return list;
  }

  @Override
  public ArrayList<ShoesVO> list_all_namesub_y(String name) {
    ArrayList<ShoesVO> list = this.list_all_namesub_y(name);
    return list;
  }
  ...
  public ArrayList<ShoesVOMenu> menu() {
    // 중분류 목록을 저장할 객체 선언
    ArrayList<ShoesVOMenu> menu = new ArrayList<ShoesVOMenu>();
    
    // 중분류 목록 로딩
    ArrayList<ShoesVO> list = this.shoesDAO.list_all_name_y();
    
    // 중분류 + 소분류 결합 
    for (ShoesVO shoesVO : list) {
      ShoesVOMenu shoesVOMenu = new ShoesVOMenu(); // 하나의 중분류 메뉴 그룹
      
      shoesVOMenu.setName(shoesVO.getTitle()); // 중분류명
      
      // 중분류명을 이용하여 소분류 목록을 가져옴.
      ArrayList<ShoesVO> list_namesub = this.shoesDAO.list_all_namesub_y(shoesVO.getTitle());
      shoesVOMenu.setList_namesub(list_namesub);
      
      if (list_namesub.size() > 0) { // 소분류가 있는 경우만 메뉴 등록
        menu.add(shoesVOMenu);
      }
